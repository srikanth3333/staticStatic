import React from 'react'

function OtherServices() {
    return (
        <>
            <div className="banner gro">
                <div className="container">
                    <div className="row align-items-center">
                        <div className="col-lg-6">
                            <h1 className="text-success">For Your Grocery Store, a Grocery Ordering App </h1>
                            <p className="banner-text">Get rid of commissions by creating your own ordering app and building your own customer base.</p>
                            <button className="btn text-white btn-success">Request Demo</button>
                        </div>
                        <div className="col-lg-6">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/grocery-aapp-phugital.png" className="img-fluid" alt="" />
                        </div>  
                    </div>
                </div>
            </div>
            <section className="developing">
                <div className="container">
                    <div className="text-center main-text">
                        <h2 className="text-success">For restaurant owners, we provide a food delivery app/ordering system.</h2>
                    </div>
                    <div className="row justify-content-center flip-box-pad">
                        <div className="col-lg-3">
                            <div className="flip-box">
                                <div className="flip-box-inner">
                                    <div className="flip-box-front">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/screen_3x-1-1-300x169.jpg" alt="Paris" className="img-fluid" />
                                       
                                    </div>
                                    <div className="flip-box-back">
                                        <p>Get rid of the hefty commissions of delivery aggregators and delight your customers</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="col-lg-3">
                            <div className="flip-box">
                                <div className="flip-box-inner">
                                    <div className="flip-box-front">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/screen_3x-1-1-300x169.jpg" alt="Paris" className="img-fluid" />
                                    </div>
                                    <div className="flip-box-back">
                                        <p>Get rid of the hefty commissions of delivery aggregators and delight your customers</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="col-lg-3">
                            <div className="flip-box">
                                <div className="flip-box-inner">
                                    <div className="flip-box-front">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/screen_3x-1-1-300x169.jpg" alt="Paris" className="img-fluid" />
                                       
                                    </div>
                                    <div className="flip-box-back">
                                        <p>Get rid of the hefty commissions of delivery aggregators and delight your customers</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            <section className="business gro-bus">
                <div className="container">
                    <div className="row justify-content-center align-items-center">
                        <div className="col-lg-6">
                            <h3 className="business-title">Our E-Commerce Solution Can Help You Increase Restaurant Sales</h3>
                            <p className="text-white">Third-party apps—many of which conceal data—interfere with the direct contact between a restaurant and its customers, resulting in expensive commissions. It's time to own a Web & Mobile Ordering System.</p>
                            <button className="btn btn-secondary">Get Started</button>
                        </div>
                        <div className="col-lg-6">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/grocery-hand-app-.png" className="img-fluid mx-auto d-block" alt="" />
                        </div>
                    </div>
                </div>
            </section>
            <section className="ecommerce gro-ecom">
                <div className="container">
                    <div className="text-center mb-5">
                        <h2 className="text-success">Our Grocery App's Highlights</h2>
                    </div>
                    <div className="row text-center justify-content-center">
                        <div className="col-lg-3">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/icons-grocery-landing-page-3-300x251.png" className="img-item" alt="" />
                            <p className="title">Create Your Own App</p>
                            <p className="text">Having your own mobile app and allowing clients to purchase straight from it can help you get more loyal customers across the world.</p>
                        </div>
                        <div className="col-lg-1"></div>
                        <div className="col-lg-3">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/icons-grocery-landing-page-3-300x251.png" className="img-item" alt="" />
                            <p className="title">Online Payments / COD</p>
                            <p className="text">Provide the flexibility to choose from multiple payment gateways or cash on delivery & process online transactions with ease.</p>
                        </div>
                        <div className="col-lg-1"></div>
                        <div className="col-lg-3">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/icons-grocery-landing-page-3-300x251.png" className="img-item" alt="" />
                            <p className="title">Instant reports</p>
                            <p className="text">Know what works the best for you with the data in hand. Your day, month, all-time instant order summary helps you to measure what is getting you maximum business.</p>
                        </div>
                        
                    </div>
                    <div className="row mt-5 text-center justify-content-center">
                        <div className="col-lg-3">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/icons-grocery-landing-page-3-300x251.png" className="img-item" alt="" />
                            <p className="title">Marketing Tools</p>
                            <p className="text">Attempt to attract maximum attention to your business by offering deals, promotions, happy hours etc.</p>
                        </div>
                        <div className="col-lg-1"></div>
                        <div className="col-lg-3">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/icons-grocery-landing-page-3-300x251.png" className="img-item" alt="" />
                            <p className="title">Manage Promotions</p>
                            <p className="text">Get more loyal customers by getting your own mobile app and letting them buy directly from it.</p>
                        </div>
                        <div className="col-lg-1"></div>
                        <div className="col-lg-3">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/icons-grocery-landing-page-3-300x251.png" className="img-item" alt="" />
                            <p className="title">WhatsApp Chat Support / Message / Call</p>
                            <p className="text">Resolve Customer Queries faster through call/chat support integrations </p>
                        </div>
                    </div>
                    
                </div>
            </section>
            <section className="partner">
                <div className="container">
                    <div className="text-center">
                        <h2 className="custom-h2">The Control Center LekkaPakka?</h2>
                    </div>
                    <div className="row my-5 align-items-center justify-content-center">
                        <div className="col-lg-12 text-center">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/grocery-system-gif-768x370.gif" alt="" className="img-fluid" />
                        </div>
                        <div className="col-lg-4">
                            <p className="custom-text">Start <span className="text-success">Selling</span>  on your own online applications without hassle free from the same day of store signup.</p>
                        </div>
                        <div className="col-lg-5">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/Copy-of-Copy-of-Website-Pages.gif" className="img-fluid" alt="" />
                        </div>
                    </div>
                    <div className="row my-5 align-items-center justify-content-center">
                        
                        <div className="col-lg-5">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/Copy-of-Copy-of-Website-Pages.gif" className="img-fluid" alt="" />
                        </div>
                        <div className="col-lg-4">
                            <p className="custom-text">Start <span className="text-success">Selling</span>  with Your Own App and Ordering System. Find New Customers Online And Build A Loyal Customer Base!</p>
                        </div>
                    </div>
                    <div className="row my-5 align-items-center justify-content-center">
                        <div className="col-lg-4">
                            <p className="custom-text">Build your own brand and acquire permanent customers world wide.</p>
                        </div>
                        <div className="col-lg-5">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/Copy-of-Copy-of-Website-Pages.gif" className="img-fluid" alt="" />
                        </div>
                    </div>
                    <div className="row my-5 align-items-center justify-content-center">
                        
                        <div className="col-lg-5">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/Copy-of-Copy-of-Website-Pages.gif" className="img-fluid" alt="" />
                        </div>
                        <div className="col-lg-4">
                            <p className="custom-text">You can use plenty of solutions for your business with lowest operational cost for your online applications.</p>
                        </div>
                    </div>
                    <div className="row my-5 align-items-center justify-content-center">
                        <div className="col-lg-4">
                            <p className="custom-text">We keep bringing plenty of solutions in application to make application friendly and to boost your business ideas.</p>
                        </div>
                        <div className="col-lg-5">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/Copy-of-Copy-of-Website-Pages.gif" className="img-fluid" alt="" />
                        </div>
                    </div>
                    <div className="row my-5 align-items-center justify-content-center">
                        
                        <div className="col-lg-5">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/Copy-of-Copy-of-Website-Pages.gif" className="img-fluid" alt="" />
                        </div>
                        <div className="col-lg-4">
                            <p className="custom-text">You can define your own promo codes, categorized products, product wise ratings, customer relationship, different delivery solutions and user friendly application for your business to avoid complicates while placing orders by customers</p>
                        </div>
                    </div>
                    <div className="row my-5 align-items-center justify-content-center">
                        <div className="col-lg-4">
                            <p className="custom-text">Avoid commission payments for the orders you're getting, lets not eat a chunk of your profits by aggregators.</p>
                        </div>
                        <div className="col-lg-5">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/Copy-of-Copy-of-Website-Pages.gif" className="img-fluid" alt="" />
                        </div>
                    </div>
                    <div className="row my-5 align-items-center justify-content-center">
                        
                        <div className="col-lg-5">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/Copy-of-Copy-of-Website-Pages.gif" className="img-fluid" alt="" />
                        </div>
                        <div className="col-lg-4">
                            <p className="custom-text">Own your online applications for your business with the cheapest one time cost that you spent</p>
                        </div>
                    </div>
                </div>
            </section>
            <section className="explore">
                <div className="contianer">
                    <div className="text-center main-text">
                        <h2 className="main-text-custom-h2 color-secondary">Explore the Solutions We Built for <br /> <span className="text-success">Our Happy Clients</span></h2>
                    </div>
                    <div id="carouselExampleIndicators" className="carousel slide" data-bs-ride="carousel">
                        <div className="carousel-indicators">
                            <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="0" className="active" aria-current="true" aria-label="Slide 1"></button>
                            <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="1" aria-label="Slide 2"></button>
                            <button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to="2" aria-label="Slide 3"></button>
                        </div>
                        <div className="carousel-inner">
                            <div className="carousel-item active">
                                <div className="row">
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-1.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-10.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                </div>
                            </div>
                            <div className="carousel-item">
                                <div className="row">
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-10.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-10.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                </div>
                            </div>
                            <div className="carousel-item">
                                <div className="row">
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-10.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                    <div className="col-lg-2">
                                        <img src="https://phygital24.com/wp-content/uploads/2021/07/phygital24-client-9.png" className="d-block w-100" alt="..." />
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
            <section className="business gro-bus">
                <div className="container">
                    <div className="row justify-content-center align-items-center">
                        <div className="col-lg-6">
                            <h3 className="business-title">You Are Few Steps Away From Taking Your Restaurant </h3>
                            <p className="text-white">Fill this form and connect with us now to get a customized solutionfor your restaurants. Go Commission-Free with your own ordering app/website and build new channels of business.</p>
                            <button className="btn btn-secondary">Get Started</button>
                        </div>
                        <div className="col-lg-6">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/grocery-laptop-app-768x625.png" className="img-fluid mx-auto d-block" alt="" />
                        </div>
                    </div>
                </div>
            </section>
            <section className="contact">
                <div className="container">
                    <div className="row mt-5 align-items-center">    
                        <div className="col-lg-6">
                            <h2 className="text-success">Contact US</h2>
                            <form action="">
                                <div className="row">
                                    <div className="col-lg-6">
                                        <div className="form-group">
                                            <label htmlFor="">First Name</label>
                                            <input type="text" className="form-control" />
                                        </div>
                                    </div>
                                    <div className="col-lg-6">
                                        <div className="form-group">
                                            <label htmlFor="">First Name</label>
                                            <input type="text" className="form-control" />
                                        </div>
                                    </div>
                                    <div className="col-lg-12">
                                        <div className="form-group">
                                            <label htmlFor="">Mobile Number</label>
                                            <input type="text" className="form-control" />
                                        </div>
                                    </div>
                                    <div className="col-lg-12">
                                        <div className="form-group">
                                            <label htmlFor="">Email</label>
                                            <input type="text" className="form-control" />
                                        </div>
                                    </div>
                                    
                                    <div className="col-lg-12">
                                        <div className="form-group">
                                            <button className="btn btn-success text-white">Submit</button>
                                        </div>
                                    </div>
                                </div>
                            </form>
                        </div>
                        <div className="col-lg-6">
                            <img src="https://phygital24.com/wp-content/uploads/2021/07/grocery-app-contact-768x505.png" className="img-fluid" alt="" />
                        </div>
                    </div>
                </div>
            </section>
        </>
    )
}

export default OtherServices;
